...
> [!TIP]
> **不要直接看代码！** 先理解分层存储的核心逻辑。
>
> LMCache 的分层存储其实就是一个"多级仓库系统"——热销商品放在门口（GPU），普通商品放在货架上（CPU），滞销商品放在仓库（Disk），极其冷门商品放在异地仓库（Remote）。每次有订单时，先去门口找，找不到再去货架，还找不到就仓库，最后才考虑重新生产（重新计算 KV）。
...
| 场景 | 行为 | 应对策略 |
|------|------|----------|
| **存储后端故障** | Redis/Mooncake 不可用 | 降级到本地 Disk，重试机制 |
| **网络分区** | Remote 传输超时 | 本地缓存优先，后台异步重试 |
| **KV 不一致** | 同一 Token 多版本共存 | 版本号标记，优先使用最新 |
| **并发写入冲突** | 多实例写入同一 Key | 乐观锁 + CAS 原子操作 |
...
生产环境如何配置各层容量？遇到 OOM 怎么排查？进入 **实战层**。
...
| 维度 | 收益 | 代价 |
|------|------|------|
| **增加 GPU 容量** | TTFT 降低 50%+ | 成本增加 3-5 倍 |
| **启用 CacheGen 压缩** | 存储 -70% | 压缩/解压 CPU 开销 +10% |
| **增大 CPU 缓存** | 命中率 +30% | 内存占用 +150% |
| **启用 Remote 存储** | 跨实例共享 | 网络延迟 +10-50ms |

**极限抉择案例**：

> 场景：电商大促期间，流量激增 10 倍，GPU 资源紧张，但 TTFT SLO 要求 < 300ms。
>
> 决策：为了保住 TTFT，选择增大 CPU 缓存到 200GB，牺牲部分内存成本。
>
> 后果：TTFT 满足 SLO，但内存成本增加 50%，需要在闲时回收缓存。
