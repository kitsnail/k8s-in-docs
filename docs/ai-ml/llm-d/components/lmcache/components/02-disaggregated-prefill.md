...
> [!TIP]
> **不要直接看配置！** 先理解 Disaggregated Prefill 的核心逻辑。
>
> Disaggregated Prefill 其实就是一个"专业分工系统"——就像餐厅有专门的"备菜厨师"（Prefiller）负责切菜、配料，和"炒菜厨师"（Decoder）负责炒菜上菜。备菜厨师专心备菜，炒菜厨师专心炒菜，传送带（NIXL）负责快速传递食材。这样分工后，即使有 100 个订单，也能快速处理，不会因为备菜堵塞了炒菜，也不会因为炒菜等备菜而延误。
...
| 场景 | 行为 | 应对策略 |
|------|------|----------|
| **NIXL 传输失败** | 跨节点 RDMA 超时 | 降级到网络传输，重试机制 |
| **PYTHONHASHSEED 不一致** | KV Cache hash 不匹配 | 强制所有节点使用相同 seed |
| **Prefiller 过载** | Prefill 队列堆积 > 100 | 自动扩容 Prefiller 数量 |
| **Decoder 空闲** | Prefill 完成但 Decoder 忙 | KV Cache 暂存 CPU，等待空闲 |
...
生产环境如何配置 XpYd？遇到传输失败怎么排查？进入 **实战层**。
...
| 维度 | 收益 | 代价 |
|------|------|------|
| **增加 Prefiller 数量** | Prefill QPS +N× | 基础设施成本 +N× |
| **启用 NIXL P2P** | 传输延迟 -90% | 需要支持 RDMA 的网络 |
| **XpYd 扩展** | 吞吐量线形扩展 | 调度复杂度增加 |

**极限抉择案例**：

> 场景：直播带货实时问答，TTFT SLO 要求 < 200ms，但预算有限，只能使用 4 张 GPU。
>
> 决策：选择 1p3d 架构（1 Prefiller + 3 Decoder），而非 2p2d。
>
> 后果：Prefill 可能成为瓶颈，TTFT 波动到 250ms，但整体吞吐量更高。
