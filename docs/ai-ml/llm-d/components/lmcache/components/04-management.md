...
理解 API 功能后，进入 **认知过渡** ——把图书馆管理逻辑转化为运维直觉。
...
> [!TIP]
> **不要直接看 API！** 先理解 Controller 的核心逻辑。
>
> Controller 其实就是一个"图书馆管理系统"——它管理所有书籍（KV Cache）的位置、负责借阅（Lookup）、清理过期书籍（Clear）、调拨书籍（Move）、预约热门书（Pin）。所有 Worker 都向它注册，它统一调度，这样你不需要记住每本书放在哪个书架，只需要问系统"我要找《三体》"，系统会告诉你"3号馆 2楼第5架"。
...
- `-->>` 表示异步返回
- `Note` 标注关键操作点
- `alt` 展示 Cache Hit/Miss 分支

### 2.4 边缘情况

| 场景 | 行为 | 应对策略 |
|------|------|----------|
| **Worker 心跳超时** | 30 秒无响应 | 自动标记离线，从索引中移除 |
| **Chunk 索引不一致** | 多 Worker 报告同一 Chunk | 使用版本号冲突解决 |
| **Move 操作超时** | P2P 传输 > 60s | 超时重试，3 次失败后报错 |
| **Pin 资源耗尽** | Pin 了 > 90% 的缓存 | 触发告警，强制释放部分 Pin |
...
生产环境如何部署 Controller？如何批量操作？进入 **实战层**。
...
| 维度 | 收益 | 代价 |
|------|------|------|
| **高频 Pin** | 热点数据永驻 | LRU 失效，整体命中率下降 |
| **批量 Move** | 快速扩缩容 | 传输期间服务受影响 |
| **实时 Lookup** | 精准监控 | Controller 负载增加 |

**极限抉择案例**：

> 场景：系统 Prompt 被频繁访问，但 Pin 后导致其他 KV 频繁被驱逐，整体命中率从 85% 下降到 60%。
>
> 决策：选择不 Pin 系统 Prompt，而是增加 CPU 缓存容量。
>
> 后果：系统 Prompt 偶尔被驱逐，需要重新计算，但整体命中率恢复到 82%。
